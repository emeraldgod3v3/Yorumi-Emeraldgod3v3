...
interface MangaCarouselSectionProps {
    title: string;
    peekData: { data: Manga[] } | null;
    fetchData: () => Promise<{ data: Manga[] }>;
    onMangaClick: (mangaId: string, autoRead?: boolean, manga?: Manga) => void;
    onViewAll?: () => void;
}

const MangaCarouselSection: React.FC<MangaCarouselSectionProps> = ({
    title,
    peekData,
    fetchData,
    onMangaClick,
    ...
    const [mangaList, setMangaList] = useState<Manga[]>(peekData?.data || []);
    const [loading, setLoading] = useState(!(peekData?.data?.length));
    ...
    return (
        <section className="container mx-auto px-4 relative z-20 mt-4 mb-12">
            <div className="flex items-center justify-between gap-3 mb-4">
                <h2 className="text-xl sm:text-2xl font-black text-white tracking-wide uppercase leading-none">{title}</h2>
...
export default MangaCarouselSection;
